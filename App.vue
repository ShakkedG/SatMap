...
async function scanSubsidenceInRect() {
  ...
  try {
    ...
    const sinking = [];
    // אם יש יותר מדי – תגביל כדי שלא ייקח נצח
    const max = 300;
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const rate = await getSubsidenceRateMmPerYear(c.lat, c.lng); // mm/yr (placeholder)
    ...
    // להצגה טובה: בניינים שוקעים באדום + תווית בעברית
    for (const item of sinking) {
      const tags = item.feature.properties || {};
      const name = buildingLabel(tags);

      // הערכת גובה אבסולוטית (placeholder) – כדי לענות על "גובה בעבר/נוכחי"
      const nowHeightM = await getEstimatedHeightMeters(item.centroid.lat, item.centroid.lng);

      const years = Math.max(
        0,
        (new Date(end.value) - new Date(start.value)) / (1000 * 60 * 60 * 24 * 365.25)
      );

      const deltaM = computeHeightDeltaMeters(item.rate, years); // שלילי = ירידה
      const pastHeightM = nowHeightM - deltaM;

      // ציור הפוליגון
      const geo = L.geoJSON(item.feature, {
        style: {
          weight: 2,
          opacity: 0.95,
          fillOpacity: 0.18,
      ...
      // בדרך כלל יש שכבה אחת (Polygon)
      let polyLayer = null;
      geo.eachLayer((l) => {
        if (!polyLayer) polyLayer = l;
      });
      if (!polyLayer) continue;

      const popupHtml = `
        <div style="font-family:system-ui;font-size:12px;">
          <div style="font-weight:700;margin-bottom:6px;">${name}</div>
          <div>שקיעה: <b>${item.rate.toFixed(2)}</b> מ״מ/שנה</div>
          <div>תקופה: ${start.value} → ${end.value} (${years.toFixed(2)} שנים)</div>
          <div style="margin-top:6px;">
            <div>גובה בעבר (משוער): <b>${pastHeightM.toFixed(2)}</b> מ׳</div>
            <div>גובה נוכחי (משוער): <b>${nowHeightM.toFixed(2)}</b> מ׳</div>
            <div>שינוי גובה (משוער): <b>${deltaM.toFixed(3)}</b> מ׳</div>
          </div>
          <div style="margin-top:6px; opacity:.8;">
            * הגבהים כאן הם הדגמה (placeholder). כדי לקבל גובה אמיתי צריך מקור נתונים מתאים (DEM/DSM או סדרת InSAR).
          </div>
        </div>
      `;

      polyLayer.bindPopup(popupHtml);
      polyLayer.bindTooltip(`${name} – שוקע`, { direction: "center", opacity: 0.9, sticky: true });
      ...
      polyLayer.on("click", zoomTo);

      // תווית "שוקע" בעברית במרכז – כדי ש"יהיה כתוב איפה כל שקיעה"
      const label = L.marker([item.centroid.lat, item.centroid.lng], {
        icon: L.divIcon({
          className: "sinkLabel",
          html: "שוקע",
          iconSize: [42, 18],
      ...
      label.on("click", zoomTo);

      polyLayer.addTo(buildingsGroup);
      label.addTo(buildingsGroup);
    ...
    status.value = `בתוך המלבן: ${subsBuildings.value.length} בניינים שוקעים (סף ${subsThreshold.value} מ״מ/שנה).`;
...
function focusSubsBuilding(b) {
  if (!b?._layer) return;
  try {
    const bounds = b._layer.getBounds?.();
    ...
    try { map.setView([b.lat, b.lng], Math.max(map.getZoom(), 18)); } catch {}
  }
  try { b._layer.openPopup(); } catch {}
}
